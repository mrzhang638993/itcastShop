...
abstract  class MysqlBaseEtl(env:StreamExecutionEnvironment)  extends  BaseEtl[RowData]{
   ...
   * */
  override def getKafkaDataStream(topic: String=GlobalConfigUtil.`input.topic.canal`): DataStreamSource[RowData] = {
    ...
    val value: DataStreamSource[RowData] = env.addSource(consumer)
    value
  }
}
